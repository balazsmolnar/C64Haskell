...
data Instruction = Instruction { iType :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState, mode :: AddressingMode }  
...
peek :: CPUState -> Memory -> Byte     
peek cpu memory = getByteFromMemory memory (word8ToInt (stackPointer cpu)+0x101)
     
compareHelper :: Memory -> Byte -> AddressingMode -> Byte -> Byte -> CPUState -> CPUState
compareHelper memory value mode byte1 byte2 cpu =
    let result = word8ToInt value - word8ToInt (getValue cpu memory mode byte1 byte2)
        s = regS cpu in
...
fBRK :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBRK cpu memory mode b1 b2 = cpuStop cpu
 
fCLC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCLC cpu memory mode b1 b2 =
     clearFlag flagCBit cpu

fCLI :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCLI cpu memory mode b1 b2 =
     clearFlag flagIBit cpu

fCLV :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCLV cpu memory mode b1 b2 =
     clearFlag flagVBit cpu

fSEC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSEC cpu memory mode b1 b2 =
     setFlag flagCBit cpu

fSEI :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSEI cpu memory mode b1 b2 =
     setFlag flagIBit cpu

fLDA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fLDA cpu memory mode b1 b2 =
    let value = getValue cpu memory mode b1 b2
        s = setZN value (regS cpu) in
    cpuNewRegA value $ cpuNewRegS s cpu 

fLDX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fLDX cpu memory mode b1 b2 =
    let value = getValue cpu memory mode b1 b2
        s = setZN value (regS cpu) in
    cpuNewRegX value $ cpuNewRegS s cpu 

fLDY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fLDY cpu memory mode b1 b2 =
    let value = getValue cpu memory mode b1 b2
        s = setZN value (regS cpu) in
    cpuNewRegY value $ cpuNewRegS s cpu 
    
fSTA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSTA cpu memory mode b1 b2 =
     let addr = getAddress cpu memory mode b1 b2 in
     setMemory addr (regA cpu) cpu

fSTX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSTX cpu memory mode b1 b2 =
     let addr = getAddress cpu memory mode b1 b2 in
     setMemory addr (regX cpu) cpu

fSTY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSTY cpu memory mode b1 b2 =
     let addr = getAddress cpu memory mode b1 b2 in
     setMemory addr (regY cpu) cpu

fTAY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTAY cpu memory mode b1 b2 =
    let value = regA cpu
        s = setZN value (regS cpu) in
    cpuNewRegY value $ cpuNewRegS s cpu 

fTYA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTYA cpu memory mode b1 b2 =
    let value = regY cpu
        s = setZN value (regS cpu) in
    cpuNewRegA value $ cpuNewRegS s cpu 

fTAX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTAX cpu memory mode b1 b2 =
    let value = regA cpu
        s = setZN value (regS cpu) in
    cpuNewRegX value $ cpuNewRegS s cpu 

fTXA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTXA cpu memory mode b1 b2 =
    let value = regX cpu
        s = setZN value (regS cpu) in
    cpuNewRegA value $ cpuNewRegS s cpu 

fTXS :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTXS cpu memory mode b1 b2 =
    let value = regX cpu in
    cpuNewSP value cpu

fTSX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fTSX cpu memory mode b1 b2 =
    let value = stackPointer cpu
        s = setZN value (regS cpu) in
    cpuNewRegX value $ cpuNewRegS s cpu 

fINY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fINY cpu memory mode b1 b2 =
    let value = regY cpu + 1
        s = setZN value (regS cpu) in
    cpuNewRegY value $ cpuNewRegS s cpu 

fDEY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fDEY cpu memory mode b1 b2 =
    let value = regY cpu - 1
        s = setZN value (regS cpu) in
    cpuNewRegY value $ cpuNewRegS s cpu 

fINX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fINX cpu memory mode b1 b2 =
    let value = regX cpu + 1
        s = setZN value (regS cpu) in
    cpuNewRegX value $ cpuNewRegS s cpu 

fINC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fINC cpu memory mode b1 b2 =
    let value = (getValue cpu memory mode b1 b2) + 1
        addr = getAddress cpu memory mode b1 b2
        s = setZN value (regS cpu) in
    setMemory addr value $ cpuNewRegS s cpu

fDEC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fDEC cpu memory mode b1 b2 =
    let value = (getValue cpu memory mode b1 b2) - 1
        addr = getAddress cpu memory mode b1 b2
        s = setZN value (regS cpu) in
    setMemory addr value $ cpuNewRegS s cpu

fDEX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fDEX cpu memory mode b1 b2 =
    let value = regX cpu - 1
        s = setZN value (regS cpu) in
    cpuNewRegX value $ cpuNewRegS s cpu 

fCPX :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCPX cpu memory mode b1 b2 =
    compareHelper memory (regX cpu) mode b1 b2 cpu

fCPY :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCPY cpu memory mode b1 b2 =
    compareHelper memory (regY cpu) mode b1 b2 cpu
    
fCMP :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fCMP cpu memory mode b1 b2 =
    compareHelper memory (regA cpu) mode b1 b2 cpu

fBNE :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBNE cpu memory mode b1 b2 =
    branchHelper (getFlag flagZBit cpu == False) b1 cpu
    
fBEQ :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBEQ cpu memory mode b1 b2 =
    branchHelper (getFlag flagZBit cpu == True) b1 cpu

fBCC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBCC cpu memory mode b1 b2 =
    branchHelper (getFlag flagCBit cpu == False) b1 cpu
    
fBCS :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBCS cpu memory mode b1 b2 =
    branchHelper (getFlag flagCBit cpu == True) b1 cpu

fBPL :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBPL cpu memory mode b1 b2 =
    branchHelper (getFlag flagNBit cpu == False) b1 cpu

fBMI :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBMI cpu memory mode b1 b2 =
    branchHelper (getFlag flagNBit cpu == True) b1 cpu

fBVC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBVC cpu memory mode b1 b2 =
    branchHelper (getFlag flagVBit cpu == False) b1 cpu

fBVS :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBVS cpu memory mode b1 b2 =
    branchHelper (getFlag flagVBit cpu == True) b1 cpu

fJMP :: CPUState -> Memory -> AddressingMode -> Byte -> Byte -> CPUState
fJMP cpu memory mode b1 b2 =
    let addr = getAddress cpu memory mode b1 b2 in
    cpuNewPointer (addr-3) cpu 

fJSR :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fJSR cpu memory mode b1 b2 =
    let addr = getAddress cpu memory Absolute b1 b2 in
    cpuNewPointer (addr-3) $ push (intToWord8(pPointer cpu+2 `mod` 256)) $ push (intToWord8((pPointer cpu+2) `div` 256)) cpu 
     
fRTS :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fRTS cpu memory mode b1 b2 =
    let low = peek cpu memory 
        hi = peek (pull cpu) memory in
    cpuNewPointer ((word8ToInt hi)*256 + (word8ToInt low)) $ (pull $ pull cpu)

fRTI :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fRTI cpu memory mode b1 b2 =
    let s = peek cpu memory 
        low = peek (pull cpu) memory
        hi = peek  (pull (pull cpu)) memory in
    cpuNewRegS s $ cpuNewPointer ((word8ToInt hi)*256 + (word8ToInt low)-1) $ (pull $ pull $ pull cpu)
    
fPHA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fPHA cpu memory mode b1 b2 =
     push (regA cpu) cpu

fPHP :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fPHP cpu memory mode b1 b2 =
     push (regS cpu) cpu

fPLA :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fPLA cpu memory mode b1 b2 =
    let value = peek cpu memory 
        s = setZN value (regS cpu) in
    cpuNewRegA value $ cpuNewRegS s (pull cpu)

fPLP :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fPLP cpu memory mode b1 b2 =
    let value = peek cpu memory in
    cpuNewRegS value $ pull cpu
    
fASL :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fASL cpu memory mode b1 b2 
        ...
        where addr = getAddress cpu memory mode b1 b2
              value = getValue cpu memory mode b1 b2
              c = value `testBit` 7
              newValue = shiftL value 1
              s = setZN newValue (regS cpu)

fROL :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fROL cpu memory mode b1 b2 
        | mode == Accumulator = cpuNewRegA newValue $ setFlagExp flagCBit c $ cpuNewRegS s cpu
        | otherwise = setMemory addr newValue $ setFlagExp flagCBit c $  cpuNewRegS s cpu
        where addr = getAddress cpu memory mode b1 b2
              value = getValue cpu memory mode b1 b2
...
fROR :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fROR cpu memory mode b1 b2 
        | mode == Accumulator = cpuNewRegA newValue $ setFlagExp flagCBit c $ cpuNewRegS s cpu
        | otherwise = setMemory addr newValue $ setFlagExp flagCBit c $  cpuNewRegS s cpu
        where addr = getAddress cpu memory mode b1 b2
              value = getValue cpu memory mode b1 b2
...
fLSR :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fLSR cpu memory mode b1 b2 
        | mode == Accumulator = cpuNewRegA newValue $ setFlagExp flagCBit c $ cpuNewRegS s cpu
        | otherwise = setMemory addr newValue $ setFlagExp flagCBit c $  cpuNewRegS s cpu
        where addr = getAddress cpu memory mode b1 b2
              value = getValue cpu memory mode b1 b2
...
fADC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fADC cpu memory mode b1 b2 =
        let value = getValue cpu memory mode b1 b2
...
fSBC :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fSBC cpu memory mode b1 b2 =
        let value = getValue cpu memory mode b1 b2
...
fBIT :: CPUState ->  Memory -> AddressingMode -> Byte -> Byte -> CPUState
fBIT cpu memory mode b1 b2 =
        let value = getValue cpu memory mode b1 b2
...
fAND cpu memory mode b1 b2 = 
        let value = (getValue cpu memory mode b1 b2) Data.Bits..&. (regA cpu)
            s = (setZN value (regS cpu)) in
            cpuNewRegA value $ cpuNewRegS s cpu

fORA cpu memory mode b1 b2 = 
        let value = (getValue cpu memory mode b1 b2) Data.Bits..|. (regA cpu)
            s = (setZN value (regS cpu)) in
            cpuNewRegA value $ cpuNewRegS s cpu

fEOR cpu memory mode b1 b2 = 
        let value = (getValue cpu memory mode b1 b2) `xor` (regA cpu)
            s = (setZN value (regS cpu)) in
            cpuNewRegA value $ cpuNewRegS s cpu

fUNDEF cpu memory mode b1 b2 = 
        cpu
        
fCLD cpu memory mode b1 b2 = 
        cpu

fSED cpu memory mode b1 b2 = 
        cpu
        
fNOP cpu memory mode b1 b2 = 
        cpu
        
getAddress :: CPUState -> Memory -> AddressingMode -> Byte -> Byte -> Int
getAddress cpu memory IndexedIndirectX byte1 byte2 =
    let addr = word8ToInt byte1 + word8ToInt (regX cpu) in
    word8ToInt (getByteFromMemory memory addr) + 256 * (word8ToInt (getByteFromMemory memory (addr+1)))
getAddress cpu memory IndirectIndexedY byte1 byte2 =
    let b1 = getByteFromMemory memory (word8ToInt byte1)
        b2 = getByteFromMemory memory (word8ToInt (byte1+1)) in
    getAddress cpu memory AbsoluteY b1 b2
getAddress cpu memory Indirect byte1 byte2 =    
    let addr = word8ToInt byte1 + 256 * word8ToInt byte2 in
    getAddress cpu memory Absolute (getByteFromMemory memory addr) (getByteFromMemory memory (addr+1))
getAddress cpu memory Absolute byte1 byte2 =    
    word8ToInt byte1 + 256 * (word8ToInt byte2)
getAddress cpu memory AbsoluteX byte1 byte2 =    
    word8ToInt byte1 + word8ToInt byte2 * 256 + word8ToInt (regX cpu)
getAddress cpu memory AbsoluteY byte1 byte2 =    
    word8ToInt byte1 + word8ToInt byte2 * 256 + word8ToInt (regY cpu)
getAddress cpu memory ZeroPage byte1 byte2 =
    word8ToInt byte1
getAddress cpu memory ZeroPageX byte1 byte2 =               
    word8ToInt $ byte1 + regX cpu
getAddress cpu memory ZeroPageY byte1 byte2 =               
    word8ToInt $ byte1 + regY cpu
getAddress cpu memory mode byte1 byte2 = error $ "Invalid addressing mode." ++ show mode

getValue :: CPUState -> Memory -> AddressingMode -> Byte -> Byte -> Byte
getValue cpu memory Immidiate byte1 byte2 = byte1
getValue cpu memory Accumulator byte1 byte2 = regA cpu
getValue cpu memory mode byte1 byte2 = getByteFromMemory memory (getAddress cpu memory mode byte1 byte2)
